...
// Importy komponentów Shadcn/Vue
...
const submitUserForm = () => {
    userForm.put(route('users.update', props.user.id), {
        onSuccess: () => {
            ...
            // Po aktualizacji danych użytkownika, nie resetujemy userForm,
            // ale odświeżamy prop 'user' na stronie, aby UI było aktualne.
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto">
            <div class="p-6 bg-white rounded-xl shadow-sm dark:bg-gray-800">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Edytuj Pracownika: {{ props.user.name }}</h3>
                ...
                <form @submit.prevent="submitUserForm" class="space-y-4">
                    ...
                    <div>
                        ...
                        <p v-if="userForm.errors.login" class="text-sm text-red-500 mt-1">{{ userForm.errors.login }}</p>
                    </div>

                    <div>
                        <Label for="email">Email</Label>
                        <Input id="email" type="email" v-model="userForm.email" />
                        <p v-if="userForm.errors.email" class="text-sm text-red-500 mt-1">{{ userForm.errors.email }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p v-if="userForm.errors.role_name" class="text-sm text-red-500 mt-1">{{ userForm.errors.role_name }}</p>
                    </div>
