...
import type { UnitOfMeasureIndexProps } from '@/types';
...
const props = defineProps<UnitOfMeasureIndexProps>();

const currentPage = ref(props.unitOfMeasures.current_page);
const currentGlobalFilter = ref(props.filter);
const showDisabled = ref(props.show_disabled);

const form = useForm({});
const isAlertDialogOpen = ref(false);
const itemToDelete = ref<Pick<UnitOfMeasure, 'id' | 'name'> | null>(null);

const confirmDelete = (item: Pick<UnitOfMeasure, 'id' | 'name'>) => {
...
const deleteItemConfirmed = () => {
    if (itemToDelete.value) {
        form.delete(route('unit-of-measures.destroy', itemToDelete.value.id), {
            onSuccess: () => {
                toast.success('Jednostka miary została pomyślnie zarchiwizowana.');
...
const restoreItem = (id: number) => {
    router.post(route('unit-of-measures.restore', id), {}, {
        onSuccess: () => {
            toast.success('Jednostka miary została pomyślnie przywrócona.');
            fetchTableData();
        },
...
watch(showDisabled, (newValue) => {
    router.get(route('unit-of-measures.index', { show_disabled: newValue }), {}, {
        preserveState: true,
        preserveScroll: true,
        only: ['unitOfMeasures', 'show_disabled'],
    });
});
...
const fetchTableData = () => {
    router.get(
        ...
        },
        {
            preserveState: true,
            preserveScroll: true,
            only: ['unitOfMeasures', 'filter', 'show_disabled', 'sort_by', 'sort_direction'],
        }
    );
};
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'flex justify-end space-x-2' }, [
                item.deleted_at === null
                    ? h(Link, { href: route('unit-of-measures.edit', item.id ) }, () => h(Button, { variant: 'outline', size: 'sm' }, { default: () => 'Edytuj' }))
                    : null,
                item.deleted_at !== null
...
<template>
    <Head title="Jednostki Miary" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto">
            <div class="p-6 bg-white rounded-xl shadow-sm dark:bg-gray-800">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Lista Jednostek Miary</h3>
                ...
                <DataTable
                    :columns="columns"
                    :data="props.unitOfMeasures.data"
                    :current-page="props.unitOfMeasures.current_page"
                    :last-page="props.unitOfMeasures.last_page"
                    :total="props.unitOfMeasures.total"
                    :per-page="props.unitOfMeasures.per_page"
                ...
                />
                <div class="mt-6">
                    <Link :href="route('unit-of-measures.create')">
                        <Button class="w-full">Dodaj Jednostkę Miary</Button>
                    </Link>
                </div>
            </div>
        </div>
        <AlertDialog :open="isAlertDialogOpen" @update:open="isAlertDialogOpen = $event">
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        Ta akcja spowoduje ukrycie jednostki miary "{{ itemToDelete?.name }}". Będzie ją można przywrócić w przyszłości.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
