...
import type { VatRateIndexProps } from '@/types';
...
const props = defineProps<VatRateIndexProps>();

const currentPage = ref(props.vatRates.current_page);
const currentGlobalFilter = ref(props.filter);
const showDisabled = ref(props.show_disabled);

const form = useForm({});
const isAlertDialogOpen = ref(false);
const itemToDelete = ref<Pick<VatRate, 'id' | 'name'> | null>(null);

const confirmDelete = (item: Pick<VatRate, 'id' | 'name'>) => {
...
const deleteItemConfirmed = () => {
    if (itemToDelete.value) {
        form.delete(route('vat-rates.destroy', itemToDelete.value.id), {
            onSuccess: () => {
                toast.success('Stawka VAT została pomyślnie zarchiwizowana.');
...
const restoreItem = (id: number) => {
    router.post(route('vat-rates.restore', id), {}, {
        onSuccess: () => {
            toast.success('Stawka VAT została pomyślnie przywrócona.');
            fetchTableData();
        },
...
watch(showDisabled, (newValue) => {
    router.get(route('vat-rates.index', { show_disabled: newValue }), {}, {
        preserveState: true,
        preserveScroll: true,
        only: ['vatRates', 'show_disabled'],
    });
});

const fetchTableData = () => {
    router.get(
        route('vat-rates.index'),
        {
        ...
        },
        {
            preserveState: true,
            preserveScroll: true,
            only: ['vatRates', 'filter', 'show_disabled', 'sort_by', 'sort_direction'],
        }
    );
};
...
const columns: ColumnDef<VatRate>[] = [
    ...
    { 
        ...
        header: 'Stawka', 
        enableSorting: true,
        cell: ({ row }) => h('span', `${row.original.rate}%`),
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'flex justify-end space-x-2' }, [
                item.deleted_at === null
                    ? h(Link, { href: route('vat-rates.edit', item.id ) }, () => h(Button, { variant: 'outline', size: 'sm' }, { default: () => 'Edytuj' }))
                    : null,
                item.deleted_at !== null
...
<template>
    <Head title="Stawki VAT" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto">
            <div class="p-6 bg-white rounded-xl shadow-sm dark:bg-gray-800">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Lista Stawek VAT</h3>
                ...
                <DataTable
                    :columns="columns"
                    :data="props.vatRates.data"
                    :current-page="props.vatRates.current_page"
                    :last-page="props.vatRates.last_page"
                    :total="props.vatRates.total"
                    :per-page="props.vatRates.per_page"
                ...
                <div class="mt-6">
                    <Link :href="route('vat-rates.create')">
                        <Button class="w-full">Dodaj Stawkę VAT</Button>
                    </Link>
                </div>
            </div>
        </div>
        <AlertDialog :open="isAlertDialogOpen" @update:open="isAlertDialogOpen = $event">
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        Ta akcja spowoduje ukrycie stawki VAT "{{ itemToDelete?.name }}". Będzie ją można przywrócić w przyszłości.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
