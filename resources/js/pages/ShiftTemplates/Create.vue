...
const submit = () => {
    form.post(route('shift-templates.store'), {
        onSuccess: () => {
            toast.success('Harmonogram zmian został pomyślnie dodany.');
            form.reset();
            form.time_from = '08:00';
            form.time_to = '16:00';
            calculateDuration();
        },
        onError: (errors) => {
            if (Object.keys(errors).length > 0) {
                ...
                toast.error('Wystąpił nieznany błąd podczas dodawania harmonogramu zmian.');
            }
        },
    });
};
...
<template>
    <Head title="Dodaj Harmonogram Zmian" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto">
            <Card class="w-full max-w-2xl mx-auto">
                <CardHeader>
                    <CardTitle>Dodaj Nową zmianę</CardTitle>
                    <CardDescription>
                        Uzupełnij szczegóły, aby zdefiniować nowy szablon zmiany pracy.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="flex justify-end gap-2">
                            ...
                            <Button type="submit" :disabled="form.processing">
                                {{ form.processing ? 'Zapisywanie...' : 'Dodaj Zmiane' }}
                            </Button>
                        </div>
                    </form>
                </CardContent>
