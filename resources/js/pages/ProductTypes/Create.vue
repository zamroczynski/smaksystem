...
import type { ProductTypeCreateProps } from '@/types';
...
const submit = () => {
    form.post(route('product-types.store'), {
        onSuccess: () => {
            toast.success('Typ produktu został pomyślnie dodany.');
            form.reset();
        },
        onError: (errors) => {
...
<template>
    <Head title="Dodaj Typ Produktu" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto">
            <div class="p-6 bg-white rounded-xl shadow-sm dark:bg-gray-800">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Dodaj Nowy Typ Produktu</h3>
                ...
                <form @submit.prevent="submit" class="space-y-4 max-w-md">
                    ...
                    <div class="flex justify-end pt-4">
                        <Button type="submit" :disabled="form.processing">Dodaj Typ Produktu</Button>
                    </div>
                </form>
            </div>
        </div>
